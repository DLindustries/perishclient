...
public class AutoUpdate {
            ...
            switch (result.getType()) {
                ...
                case OLD:
                    Utils.sendMessageAnyWay(ChatFormatting.RED + "You are not at latest version." +
                            ChatFormatting.RESET + " current: " + "V6" + "  latest: " + result.getLatestVersion());
    ...
    public static void update() {
        ...
        try (CloseableHttpClient httpClient = HttpClientBuilder.create()
            ...
            output.flush();
            output.close();
            Utils.sendMessage(ChatFormatting.GREEN + "Download success! Restart minecraft launcher to restart perishclient");
        } catch (Exception e) {
    ...
    private static @NotNull Result checkVersion() {
        try (CloseableHttpClient httpClient = HttpClientBuilder.create().build()) {
            HttpGet httpGet = new HttpGet("https://api.github.com/repos/DLindustries/perishclient/releases/latest");
            CloseableHttpResponse response = httpClient.execute(httpGet);

            if (response.getStatusLine().getStatusCode() != 200) {
                Utils.sendMessage(ChatFormatting.RED + "Fail to check latest version. Are you even connected to the internet?");
